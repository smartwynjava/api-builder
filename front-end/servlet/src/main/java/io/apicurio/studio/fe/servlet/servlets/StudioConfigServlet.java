...
//            HttpSession session = request.getSession();
            ...
            auth.setToken("Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJpZCI6InRlc3QifQ.BaGxNQFedbRaRJUZkeRaq3Wo_4elMmmS7FKSqgAB80dy3aveXGhSbOCZUt9JkNsBijZYexxhp5TRw2ffkS1v8A");
            User user = new User();
...
//                url = request.getRequestURL().toString();
                url = new URI("http://localhost:8091").toString();
            }
            return url;
...
//                url = request.getRequestURL().toString().replaceFirst("http", "ws");
                url = new URI("ws://localhost:8092").toString();
            }
            return url;
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
