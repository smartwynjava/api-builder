...
import { GenerateCodeWizardModel } from "../pages/apis/{apiId}/_components/generate-code.wizard";
...
export class ApisService extends AbstractHubService {
     ...
     * @param data
     */
    public generateCode(designId: string | number, data: GenerateCodeWizardModel): Observable<HttpResponse<Blob>> {
        console.info("[ApisService] Generating code for Artifact %s", designId);
        let generateCodeUrl: string = this.endpoint("/code/:designId", {
            designId: designId
        });
        let options: any = this.options({ "Accept": "*/*", "Content-Type": "application/json" });

        return this.http.post(generateCodeUrl, data, {
